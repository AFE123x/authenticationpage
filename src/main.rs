...
async fn handle_register(jar: CookieJar, Form(form): Form<RegisterForm>) -> impl IntoResponse {
    ...
    // ← Log lock acquisition so you can spot contention in traces
    info!(
    ...
    if users.values().any(|u| u.email == form.email) {
        warn!(
            ...
            form.username
            // ← deliberately omit the email itself from logs to limit PII exposure
        );
        return (
            StatusCode::CONFLICT,
...
mod tests {
    ...
    async fn test_session_lifecycle() {
        ...
        assert!(session.is_some(), "Session should be valid immediately after creation");
